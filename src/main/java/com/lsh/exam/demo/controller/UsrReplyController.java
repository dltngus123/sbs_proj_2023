...
import com.lsh.exam.demo.service.ReplyService;
...
public class UsrReplyController {
	...
	public String doWrite(String relTypeCode, int relId, String body, String replaceUri) {
		...
		if ( Ut.empty(relTypeCode)) {
			return rq.jsHistoryBack("relTypeCode(을)를 입력 해주세요.");
		}
		
		if ( Ut.empty(relId)) {
			return rq.jsHistoryBack("relId(을)를 입력 해주세요.");
		}
		
		if ( Ut.empty(body)) {
			return rq.jsHistoryBack("body(을)를 입력 해주세요.");
		}
	...
	public String doDelete( int id, String replaceUri) {
		...
		if ( Ut.empty(id)) {
			return rq.jsHistoryBack("id(을)를 입력 해주세요.");
		}
		...
		if (reply.isExtra__actorCanDelete() == false) {
			return rq.jsHistoryBack(Ut.f("%d번 댓글 삭제 권한이 없습니다.", id));
		}
	...
	public String showModify( Model model, int id) {
		...
		if (reply.isExtra__actorCanDelete() == false) {
			return rq.jsHistoryBack(Ut.f("%d번 댓글 수정 권한이 없습니다.", id));
		}
